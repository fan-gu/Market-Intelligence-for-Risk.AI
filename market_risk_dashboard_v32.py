"""MIRAI V32: granular book-level risk aggregation on the V31 foundation."""
...
st.set_page_config(page_title="MIRAI | Market Intelligence for Risk AI | V32", page_icon=":material/monitoring:", layout="wide")

try:
    import market_risk_agent_v32 as v32
    v29 = v32.v29
...
if portfolio_df.empty:
    ...
    st.stop()
_, stress_metadata = v29.build_supplied_stress_frame(selected_as_of_date)
...
base_limit_rows = v29.evaluate_all_limits()["limits"]
scenario_columns = [definition["column"] for definition in v29.STRESS_SCENARIO_DEFINITIONS.values()]
...
scope_limit_evaluation = {
    ...
    "usage_note": "V32 limits follow the selected explicit book perimeter; warning starts at 80% and breach at 100%.",
}
...
for scenario, definition in v29.STRESS_SCENARIO_DEFINITIONS.items():
...
if page == "Dashboard":
    ...
    with st.container(border=True):
        ...
        if not attribution_candidates.empty:
            ...
            factor_changes = []
            for factor, columns in v29.v23.VAR_ATTRIBUTION_GROUPS.items():
    ...
    desk_history["explained_pnl"] = desk_history[v29.DRIVER_COLUMNS].sum(axis=1)
    ...
    for scenario in stress_numeric:
        ...
        scenario_limit = float(v29.STRESS_SCENARIO_LIMITS[scenario]) * allocation_weight
...
FastAPI risk-run boundary [implemented V31]
    ...
    capability_rows = pd.DataFrame([
        ...
        {"Capability": "Granular book-risk facts", "Status": "Implemented in V32", "Governance purpose": "Explicit book/date records replace fixed dashboard allocations and reconcile to the bank total"},
...
- Human approval is required for limit decisions, escalations and risk actions."""
    ...
    with st.container(border=True):
        ...
        st.success("Every additive daily measure reconciles to the V31 bank-wide source within floating-point tolerance.", icon=":material/check_circle:")
    with st.container(border=True):
