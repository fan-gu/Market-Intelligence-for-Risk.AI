"""M.R. AI Agent V24: residual-focused P&L, richer sensitivities, and consolidated controls."""

import altair as alt
import pandas as pd
import streamlit as st

st.set_page_config(page_title="M.R. AI Agent | V24", page_icon=":material/monitoring:", layout="wide")
...
st.session_state.setdefault("v24_active_page", "Overview")


def select_dashboard_page():
    st.session_state.v24_active_page = st.session_state.v24_navigation


def select_agent_page():
    st.session_state.v24_active_page = "Ask MR Agent"
...
stress_evolution = v24.get_stress_evolution()

portfolio_scope = v24.v14.get_portfolio_scope()
...
books, _ = v24.v15.build_hierarchy()
...
page = st.session_state.v24_active_page
...
if portfolio_df.empty:
    ...
    st.stop()
stress_frame, stress_metadata = v24.build_supplied_stress_frame(selected_as_of_date)
...
st.info(
    "V24 hierarchy filters apply transparent synthetic allocations for exploration. "
...
if page == "Overview":
    ...
    st.header("VaR")
    var_change_summary = v24.get_var_change_summary(selected_as_of_date)
    ...
    with st.container(border=True):
        ...
        attribution_horizon = st.segmented_control(
            ...
            default="Daily",
            required=True,
            key="v24_var_attribution_horizon",
        )
        var_attribution = v24.get_var_change_attribution(
            selected_as_of_date,
    ...
    pla_history = v24.build_pla_demo_history()
    pla_evaluation = v24.evaluate_pla_test()
    ...
    pnl_alert_evaluation = v24.evaluate_pnl_explain_alerts()
    ...
    st.info(
        "The 250-day desk history is deterministic synthetic V24 demo data. "
    ...
    with st.container(border=True):
        ...
        driver_contributions = [
            ...
            for column in v24.DRIVER_COLUMNS
        ]
        st.altair_chart(
            build_waterfall_chart(
                driver_contributions,
    ...
    sensitivities = v24.get_market_sensitivities()
    ...
    def sensitivity_tenor_matrix(measure):
        detail = rates_frame.loc[rates_frame["measure"] == measure].pivot_table(
        ...
        detail = detail[["currency", "curve_type", "curve"] + tenor_order + ["Total"]]
        return detail.sort_values(["currency", "curve_type", "curve"])
    ...
    material_evaluation = v24.get_material_stress_scenarios(selected_as_of_date)
    stress_limit_monitor = v24.get_stress_limit_monitor(selected_as_of_date)
    ...
    with st.expander("Scenario catalogue, including Extreme configurations", icon=":material/assignment:"):
        scenario_catalog = pd.DataFrame(v24.get_stress_scenario_catalog())
    ...
    st.header("Controls")
    limit_evaluation = v24.evaluate_all_limits()
    ...
    materiality = v24.detect_material_risk_movements(selected_as_of_date)
    ...
    with st.container(border=True):
        ...
        if question:
            ...
            with st.status("Running the risk investigation...", expanded=True) as status:
                try:
                    answer = v24.ask_risk_agent(question)
                except Exception as error:
    ...
    with st.expander("Recent investigation memory", icon=":material/history:"):
        memory = v24.v9.get_recent_investigation_context()
