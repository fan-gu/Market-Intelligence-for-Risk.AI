...
st.set_page_config(page_title="M.R. AI Agent | V18", page_icon=":material/monitoring:", layout="wide")

try:
    import market_risk_agent_v18 as v18
...
stress_evolution = v18.get_stress_evolution()

portfolio_scope = v18.v14.get_portfolio_scope()
...
stress_frame, stress_metadata = v18.build_supplied_stress_frame(selected_as_of_date)

books, _ = v18.v15.build_hierarchy()
...
st.info("V18 hierarchy filters apply transparent synthetic allocations for exploration. They are not trade-level revaluation or additive VaR.", icon=":material/info:")
...
if page == "Overview":
    ...
    sensitivities = v18.get_market_sensitivities()
    ...
    st.header("Controls")
    limit_evaluation = v18.evaluate_all_limits()
        ...
        if question:
            ...
            with st.chat_message("assistant", avatar=":material/analytics:"):
                with st.status("Running the risk investigation...", expanded=True) as status:
                    try:
                        answer = v18.ask_risk_agent(question)
                    except Exception as error:
    ...
    with st.expander("Recent investigation memory", icon=":material/history:"):
        memory = v18.v9.get_recent_investigation_context()
