"""M.R. AI Agent V28: residual-focused P&L, richer sensitivities, and consolidated controls."""
...
st.set_page_config(page_title="M.R. AI Agent | V28", page_icon=":material/monitoring:", layout="wide")

try:
    import market_risk_agent_v28 as v28
...
st.session_state.setdefault("v28_active_page", "Overview")


def select_dashboard_page():
    st.session_state.v28_active_page = st.session_state.v28_navigation


def select_agent_page():
    st.session_state.v28_active_page = "Ask MR Agent"
...
stress_evolution = v28.get_stress_evolution()

portfolio_scope = v28.v14.get_portfolio_scope()
...
books, _ = v28.v15.build_hierarchy()
...
page = st.session_state.v28_active_page
...
if portfolio_df.empty:
    ...
    st.stop()
stress_frame, stress_metadata = v28.build_supplied_stress_frame(selected_as_of_date)
...
header_limit_rows = v28.evaluate_all_limits()["limits"]
header_stress_rows = v28.get_stress_limit_monitor(selected_as_of_date)["scenarios"]
header_breaches = (
...
if page == "Overview":
    ...
    st.header("VaR")
    var_change_summary = v28.get_var_change_summary(selected_as_of_date)
    ...
    with st.container(border=True):
        ...
        attribution_horizon = st.segmented_control(
            ...
            default="Daily",
            required=True,
            key="v28_var_attribution_horizon",
        )
        var_attribution = v28.get_var_change_attribution(
            selected_as_of_date,
    ...
    pla_history = v28.build_pla_demo_history()
    ...
    pla_ks = v28.v19._empirical_ks_statistic(
        desk_history["hypothetical_pnl"], desk_history["risk_theoretical_pnl"]
    )
    pla_zone = v28.v19._pla_zone(pla_correlation, pla_ks)
    pla_consequence = {
    ...
    st.info(
        "The 250-day desk history is deterministic synthetic V28 demo data. "
    ...
    with st.container(border=True):
        ...
        driver_contributions = [
            ...
            for column in v28.DRIVER_COLUMNS
        ]
        st.altair_chart(
            build_waterfall_chart(
                driver_contributions,
    ...
    sensitivities = v28.get_market_sensitivities()
    ...
    with st.container(border=True):
        ...
        delta_result = v28.get_delta_curve_tenor_summary(selected_currencies)
    ...
    with st.container(border=True):
        ...
        surface_result = v28.get_ir_vega_surface(selected_currencies)
        ...
        st.caption(
            ...
            "The V28 prototype preserves each currency's source Vega; production feeds should provide native surface nodes."
        )
    ...
    material_evaluation = v28.get_material_stress_scenarios(selected_as_of_date)
    stress_limit_monitor = v28.get_stress_limit_monitor(selected_as_of_date)
    ...
    with st.expander("Scenario catalogue, including Extreme configurations", icon=":material/assignment:"):
        scenario_catalog = pd.DataFrame(v28.get_stress_scenario_catalog())
    ...
    st.header("Controls")
    limit_evaluation = v28.evaluate_all_limits()
    ...
    daily_brief = v28.generate_daily_risk_brief(selected_as_of_date)
    ...
    materiality = v28.detect_material_risk_movements(selected_as_of_date)
    ...
    with st.container(border=True):
        ...
        pnl_alert_evaluation = v28.evaluate_pnl_explain_alerts()
    ...
    with st.container(border=True):
        ...
        if question:
            ...
            with st.status("Running the risk investigation...", expanded=True) as status:
                try:
                    answer = v28.ask_risk_agent(question)
                except Exception as error:
    ...
    with st.expander("Recent investigation memory", icon=":material/history:"):
        memory = v28.v9.get_recent_investigation_context()
