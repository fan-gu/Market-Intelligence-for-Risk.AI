"""M.R. AI Agent V25: residual-focused P&L, richer sensitivities, and consolidated controls."""

import altair as alt
import pandas as pd
import streamlit as st

st.set_page_config(page_title="M.R. AI Agent | V25", page_icon=":material/monitoring:", layout="wide")

try:
    import market_risk_agent_v25 as v25
...
st.session_state.setdefault("v25_active_page", "Overview")


def select_dashboard_page():
    st.session_state.v25_active_page = st.session_state.v25_navigation


def select_agent_page():
    st.session_state.v25_active_page = "Ask MR Agent"
...
stress_evolution = v25.get_stress_evolution()

portfolio_scope = v25.v14.get_portfolio_scope()
...
books, _ = v25.v15.build_hierarchy()
...
page = st.session_state.v25_active_page
...
if portfolio_df.empty:
    ...
    st.stop()
stress_frame, stress_metadata = v25.build_supplied_stress_frame(selected_as_of_date)
...
header_limit_rows = v25.evaluate_all_limits()["limits"]
header_stress_rows = v25.get_stress_limit_monitor(selected_as_of_date)["scenarios"]
header_breaches = (
...
if page == "Overview":
    ...
    st.header("VaR")
    var_change_summary = v25.get_var_change_summary(selected_as_of_date)
    ...
    with st.container(border=True):
        ...
        attribution_horizon = st.segmented_control(
            ...
            default="Daily",
            required=True,
            key="v25_var_attribution_horizon",
        )
        var_attribution = v25.get_var_change_attribution(
            selected_as_of_date,
    ...
    pla_history = v25.build_pla_demo_history()
    ...
    pla_ks = v25.v19._empirical_ks_statistic(
        desk_history["hypothetical_pnl"], desk_history["risk_theoretical_pnl"]
    )
    pla_zone = v25.v19._pla_zone(pla_correlation, pla_ks)
    pla_consequence = {
    ...
    st.info(
        "The 250-day desk history is deterministic synthetic V25 demo data. "
    ...
    with st.container(border=True):
        ...
        driver_contributions = [
            ...
            for column in v25.DRIVER_COLUMNS
        ]
        st.altair_chart(
            build_waterfall_chart(
                driver_contributions,
    ...
    sensitivities = v25.get_market_sensitivities()
    ...
    def currency_sensitivity_chart(measure, unit):
        chart_data = (
    ...
    def sensitivity_tenor_matrix(measure):
        measure_frame = rates_frame.loc[rates_frame["measure"] == measure].copy()
        detail = measure_frame.pivot_table(
        ...
        for tenor in tenor_order:
            if tenor not in detail:
                detail[tenor] = 0.0
        detail["Total"] = detail[tenor_order].sum(axis=1)
        detail["Gross exposure"] = float("nan")
        detail["Limit"] = float("nan")
        detail["Consumption"] = float("nan")
        detail["Status"] = ""

        rule = v25.SENSITIVITY_LIMITS[measure]
        gross_exposure = float(measure_frame["value"].abs().sum())
        consumption = 0.0 if rule["limit"] == 0 else gross_exposure / rule["limit"] * 100.0
        gross_by_tenor = measure_frame.groupby("tenor")["value"].apply(lambda values: values.abs().sum())
        total_row = {
            "currency": "TOTAL",
            "curve_type": "Limit control",
            "curve": "Gross absolute exposure",
            **{tenor: float(gross_by_tenor.get(tenor, 0.0)) for tenor in tenor_order},
            "Total": gross_exposure,
            "Gross exposure": gross_exposure,
            "Limit": float(rule["limit"]),
            "Consumption": consumption,
            "Status": v25.v24._limit_status(consumption),
        }
        detail = detail.sort_values(["currency", "curve_type", "curve"])
        detail = pd.concat([detail, pd.DataFrame([total_row])], ignore_index=True)
        return detail[
            ["currency", "curve_type", "curve"] + tenor_order
            + ["Total", "Gross exposure", "Limit", "Consumption", "Status"]
        ]
    rate_sections = [
        ("IR Delta (DV01)", "IR Delta", "EUR / bp"),
        ("IR Gamma", "IR Gamma", "EUR / bp²"),
        ("Vega", "IR Vega", "EUR / vol point"),
    ]
    for measure, label, unit in rate_sections:
        with st.container(border=True):
            st.subheader(f"{label} by currency ({unit})")
            st.altair_chart(currency_sensitivity_chart(measure, unit))
            st.markdown("**Curve and tenor detail**")
            matrix = sensitivity_tenor_matrix(measure)
            number_columns = {
                tenor: st.column_config.NumberColumn(tenor, format="%.0f")
                for tenor in tenor_order + ["Total"]
            }
            st.dataframe(
                matrix,
                hide_index=True,
                column_config={
                    ...
                    "curve": "Curve",
                    **number_columns,
                    "Gross exposure": st.column_config.NumberColumn(format="%.0f"),
                    "Limit": st.column_config.NumberColumn(format="%.0f"),
                    "Consumption": st.column_config.ProgressColumn(format="%.1f%%", min_value=0, max_value=160),
                    "Status": "Limit status",
                },
            )
    ...
    st.caption(
        "IR Delta is DV01 per +1 bp move in each tenor bucket. Each rate chart shows one net bar per currency; "
        "the matrix immediately below provides the curve-family, curve and tenor drill-down. "
    ...
    material_evaluation = v25.get_material_stress_scenarios(selected_as_of_date)
    stress_limit_monitor = v25.get_stress_limit_monitor(selected_as_of_date)
    ...
    with st.expander("Scenario catalogue, including Extreme configurations", icon=":material/assignment:"):
        scenario_catalog = pd.DataFrame(v25.get_stress_scenario_catalog())
    ...
    st.header("Controls")
    limit_evaluation = v25.evaluate_all_limits()
    ...
    daily_brief = v25.generate_daily_risk_brief(selected_as_of_date)
    ...
    materiality = v25.detect_material_risk_movements(selected_as_of_date)
    ...
    with st.container(border=True):
        ...
        pnl_alert_evaluation = v25.evaluate_pnl_explain_alerts()
    ...
    with st.container(border=True):
        ...
        if question:
            ...
            with st.status("Running the risk investigation...", expanded=True) as status:
                try:
                    answer = v25.ask_risk_agent(question)
                except Exception as error:
    ...
    with st.expander("Recent investigation memory", icon=":material/history:"):
        memory = v25.v9.get_recent_investigation_context()
