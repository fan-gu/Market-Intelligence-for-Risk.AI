"""M.R. AI Agent V26: residual-focused P&L, richer sensitivities, and consolidated controls."""

import altair as alt
import pandas as pd
import streamlit as st

st.set_page_config(page_title="M.R. AI Agent | V26", page_icon=":material/monitoring:", layout="wide")

try:
    import market_risk_agent_v26 as v26
...
st.session_state.setdefault("v26_active_page", "Overview")


def select_dashboard_page():
    st.session_state.v26_active_page = st.session_state.v26_navigation


def select_agent_page():
    st.session_state.v26_active_page = "Ask MR Agent"
...
stress_evolution = v26.get_stress_evolution()

portfolio_scope = v26.v14.get_portfolio_scope()
...
books, _ = v26.v15.build_hierarchy()

header_background = "#0F172A" if st.context.theme.type == "dark" else "#FFFFFF"
header_border = "#334155" if st.context.theme.type == "dark" else "#D0D5DD"
st.html(
    f"""
    <style>
...
page = st.session_state.v26_active_page
...
if portfolio_df.empty:
    ...
    st.stop()
stress_frame, stress_metadata = v26.build_supplied_stress_frame(selected_as_of_date)
...
header_limit_rows = v26.evaluate_all_limits()["limits"]
header_stress_rows = v26.get_stress_limit_monitor(selected_as_of_date)["scenarios"]
header_breaches = (
...
if page == "Overview":
    ...
    st.header("VaR")
    var_change_summary = v26.get_var_change_summary(selected_as_of_date)
    ...
    with st.container(border=True):
        ...
        attribution_horizon = st.segmented_control(
            ...
            default="Daily",
            required=True,
            key="v26_var_attribution_horizon",
        )
        var_attribution = v26.get_var_change_attribution(
            selected_as_of_date,
    ...
    pla_history = v26.build_pla_demo_history()
    ...
    pla_ks = v26.v19._empirical_ks_statistic(
        desk_history["hypothetical_pnl"], desk_history["risk_theoretical_pnl"]
    )
    pla_zone = v26.v19._pla_zone(pla_correlation, pla_ks)
    pla_consequence = {
    ...
    st.info(
        "The 250-day desk history is deterministic synthetic V26 demo data. "
    ...
    with st.container(border=True):
        ...
        driver_contributions = [
            ...
            for column in v26.DRIVER_COLUMNS
        ]
        st.altair_chart(
            build_waterfall_chart(
                driver_contributions,
    ...
    sensitivities = v26.get_market_sensitivities()
    ...
    with st.container(border=True):
        st.subheader("IR Delta limits by currency (EUR / bp)")
        delta_summary = v26.get_delta_limit_summary(selected_currencies)
        delta_table = pd.DataFrame(delta_summary["rows"])
        delta_table["net_delta"] *= allocation_weight
        delta_table["gross_delta"] *= allocation_weight
        delta_table["net_pct"] = delta_table["net_delta"].abs() / delta_table["net_limit"] * 100.0
        delta_table["gross_pct"] = delta_table["gross_delta"] / delta_table["gross_limit"] * 100.0

        delta_chart = delta_table.loc[delta_table["currency"] != "TOTAL"].melt(
            id_vars="currency",
            value_vars=["net_delta", "gross_delta"],
            var_name="Delta type",
            value_name="value",
        )
        delta_chart["Delta type"] = delta_chart["Delta type"].map(
            {"net_delta": "Net Delta", "gross_delta": "Gross Delta"}
        )
        st.altair_chart(
            ...
            .encode(
                ...
                xOffset="Delta type:N",
                y=alt.Y("value:Q", title="EUR / bp", scale=alt.Scale(zero=True)),
                color=alt.Color(
                    "Delta type:N",
                    scale=alt.Scale(
                        domain=["Net Delta", "Gross Delta"],
                        range=["#2F6BFF", "#8FB5FF"],
                    ),
                ),
                tooltip=[
                    alt.Tooltip("currency:N", title="Currency"),
                    alt.Tooltip("Delta type:N", title="Measure"),
                    alt.Tooltip("value:Q", title="Delta", format=",.0f"),
                ],
            )
        ...
        st.dataframe(
            delta_table[[
                "currency", "net_delta", "net_limit", "net_pct",
            ...
            column_config={
                ...
                "net_pct": st.column_config.ProgressColumn("%", format="%.1f%%", min_value=0, max_value=160),
                "gross_delta": st.column_config.NumberColumn("Gross Delta", format="%.0f"),
                "gross_limit": st.column_config.NumberColumn("Limit", format="%.0f"),
                "gross_pct": st.column_config.ProgressColumn("%", format="%.1f%%", min_value=0, max_value=160),
            },
        )
        st.caption(delta_summary["usage_note"])
    ...
    with st.container(border=True):
        ...
        gamma_table = pd.concat([
            gamma_table,
            pd.DataFrame([{"Currency": "TOTAL", "IR Gamma": gamma_table["IR Gamma"].sum()}]),
        ], ignore_index=True)
        st.altair_chart(
            alt.Chart(gamma_table.loc[gamma_table["Currency"] != "TOTAL"])
            .mark_bar(size=52)
            .encode(
        ...
        st.dataframe(
            gamma_table,
            hide_index=True,
            column_config={"IR Gamma": st.column_config.NumberColumn(format="%.0f")},
        )
        st.caption(
            "IR Gamma is the signed sum across all displayed curve and tenor nodes. "
            "It is informational and has no limit or consumption in V26."
        )

    with st.container(border=True):
        st.subheader("IR Vega surface (EUR / vol point)")
        surface_result = v26.get_ir_vega_surface(selected_currencies)
        surface_frame = pd.DataFrame(surface_result["surface"])
        surface_frame["value"] *= allocation_weight
        surface_total = (
            surface_frame.groupby(["option_expiry", "underlying_tenor"], as_index=False)["value"]
            .sum()
        )
        heatmap = (
            alt.Chart(surface_total)
            .mark_rect(cornerRadius=5)
            .encode(
                x=alt.X(
                    "underlying_tenor:N",
                    title="Underlying swap tenor",
        ...
        labels = (
            alt.Chart(surface_total)
            .mark_text(fontSize=17, fontWeight=600, color="#111827")
            .encode(
                x=alt.X("underlying_tenor:N", sort=surface_result["underlying_tenors"]),
                y=alt.Y("option_expiry:N", sort=surface_result["option_expiries"]),
                text=alt.Text("value:Q", format=",.0f"),
            )
        )
        st.altair_chart(heatmap + labels)

        vega_table = surface_frame.pivot_table(
            index="currency",
            columns="surface_node",
            values="value",
            aggfunc="sum",
            fill_value=0,
        ).reset_index()
        node_order = [
            f"{expiry} x {tenor}"
            for expiry in surface_result["option_expiries"]
            for tenor in surface_result["underlying_tenors"]
        ]
        for node in node_order:
            if node not in vega_table:
                vega_table[node] = 0.0
        vega_table["Total"] = vega_table[node_order].sum(axis=1)
        total_row = {
            "currency": "TOTAL",
            **{node: vega_table[node].sum() for node in node_order},
            "Total": vega_table["Total"].sum(),
        }
        vega_table = pd.concat([vega_table, pd.DataFrame([total_row])], ignore_index=True)
        st.dataframe(
            vega_table[["currency"] + node_order + ["Total"]],
            hide_index=True,
            column_config={
                "currency": st.column_config.TextColumn("Currency", pinned=True),
                **{
                    node: st.column_config.NumberColumn(node, format="%.0f")
                    for node in node_order
                },
                "Total": st.column_config.NumberColumn(format="%.0f"),
            },
        )
        st.caption(surface_result["usage_note"])
    ...
    st.caption(
        "IR Delta is DV01 per +1 bp move in each tenor bucket. Net Delta preserves signs; "
        "Gross Delta sums absolute node values. IR Vega is displayed as an expiry-by-underlying-tenor "
        "surface. FX Delta is measured per +1% move in the quoted pair."
    )
elif page == "Stress":
    st.header("Stress")
    ...
    material_evaluation = v26.get_material_stress_scenarios(selected_as_of_date)
    stress_limit_monitor = v26.get_stress_limit_monitor(selected_as_of_date)
    ...
    with st.expander("Scenario catalogue, including Extreme configurations", icon=":material/assignment:"):
        scenario_catalog = pd.DataFrame(v26.get_stress_scenario_catalog())
    ...
    st.header("Controls")
    limit_evaluation = v26.evaluate_all_limits()
    ...
    daily_brief = v26.generate_daily_risk_brief(selected_as_of_date)
    ...
    materiality = v26.detect_material_risk_movements(selected_as_of_date)
    ...
    with st.container(border=True):
        ...
        pnl_alert_evaluation = v26.evaluate_pnl_explain_alerts()
    ...
    with st.container(border=True):
        ...
        if question:
            ...
            with st.status("Running the risk investigation...", expanded=True) as status:
                try:
                    answer = v26.ask_risk_agent(question)
                except Exception as error:
    ...
    with st.expander("Recent investigation memory", icon=":material/history:"):
        memory = v26.v9.get_recent_investigation_context()
