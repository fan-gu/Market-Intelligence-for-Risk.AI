"""Market Risk AI V16: restored risk-manager pages with hierarchy allocation filters."""

import pandas as pd
import streamlit as st

st.set_page_config(page_title="Market Risk AI | V16", page_icon=":material/monitoring:", layout="wide")

try:
    import market_risk_agent_v16 as v16
...
portfolio_scope = v16.v14.get_portfolio_scope()
portfolio_ids = [row["portfolio_id"] for row in portfolio_scope["portfolios"]]

st.title("Market Risk AI")
st.caption(f"Validated demo risk run · Data as of {current_risk['date']} · V16")
nav, date_control = st.columns([5, 2], vertical_alignment="bottom")
with nav:
...
books, _ = v16.v15.build_hierarchy()
...
st.info("V16 hierarchy filters apply transparent synthetic allocations for exploration. They are not trade-level revaluation or additive VaR.", icon=":material/info:")
...
if page == "Overview":
    ...
    st.header("Greek sensitivities")
    sensitivities = v16.get_greek_sensitivities()
    st.caption(sensitivities["usage_note"])
    sensitivity_frame = pd.DataFrame(sensitivities["sensitivities"].items(), columns=["Greek", "P&L-driver proxy"])
    with st.container(horizontal=True):
        for greek, value in sensitivities["sensitivities"].items():
            st.metric(greek, amount(value), border=True)
    with st.container(border=True):
        st.subheader("Current sensitivity proxies")
        display_amount_table(sensitivity_frame, ["P&L-driver proxy"])
elif page == "Stress":
    st.header("Stress")
        ...
        if question:
            ...
            with st.chat_message("assistant", avatar=":material/analytics:"):
                with st.status("Running the risk investigation...", expanded=True) as status:
                    try:
                        answer = v16.ask_risk_agent(question)
                    except Exception as error:
    ...
    with st.expander("Recent investigation memory", icon=":material/history:"):
        memory = v16.v9.get_recent_investigation_context()
