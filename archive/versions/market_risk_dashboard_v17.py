"""M.R. AI Agent V17: governed limits and corrected risk semantics."""

import pandas as pd
import streamlit as st

st.set_page_config(page_title="M.R. AI Agent | V17", page_icon=":material/monitoring:", layout="wide")

try:
    import market_risk_agent_v17 as v17
...
stress_evolution = v17.get_stress_evolution()

portfolio_scope = v17.v14.get_portfolio_scope()
...
stress_frame, stress_metadata = v17.build_supplied_stress_frame(selected_as_of_date)

books, _ = v17.v15.build_hierarchy()
...
st.info("V17 hierarchy filters apply transparent synthetic allocations for exploration. They are not trade-level revaluation or additive VaR.", icon=":material/info:")
...
if page == "Overview":
    ...
    sensitivities = v17.get_market_sensitivities()
    ...
    with st.container(horizontal=True):
        st.metric("IR Delta (DV01)", f"{amount(ir_delta)} EUR / bp", border=True)
        st.metric("IR Gamma", f"{amount(gamma)} EUR / bp²", border=True)
        st.metric("Vega", f"{amount(vega)} EUR / vol point", border=True)
        st.metric("Theta", f"{amount(theta)} EUR / day", border=True)
    ...
    st.header("Controls")
    limit_evaluation = v17.evaluate_limit_breaches()
    limit_frame = pd.DataFrame(limit_evaluation["limits"])
    limit_frame["exposure"] = limit_frame["exposure"] * allocation_weight
    limit_frame["limit"] = limit_frame["limit"] * allocation_weight
    governed_limit = limit_frame.iloc[0]
    with st.container(horizontal=True):
        st.metric("VaR limit status", governed_limit["status"], border=True)
        st.metric("Utilisation", percentage(governed_limit["utilization_pct"]), border=True)
        st.metric("Owner", governed_limit["owner"], border=True)
        st.metric("Escalation", governed_limit["escalation_status"], border=True)
    with st.container(border=True):
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "warning_threshold_pct": st.column_config.NumberColumn("Warning threshold", format="%.1f%%"),
                "critical_threshold_pct": st.column_config.NumberColumn("Critical threshold", format="%.1f%%"),
            },
        )
        ...
        if question:
            ...
            with st.chat_message("assistant", avatar=":material/analytics:"):
                with st.status("Running the risk investigation...", expanded=True) as status:
                    try:
                        answer = v17.ask_risk_agent(question)
                    except Exception as error:
    ...
    with st.expander("Recent investigation memory", icon=":material/history:"):
        memory = v17.v9.get_recent_investigation_context()
