"""M.R. AI Agent V22: residual-focused P&L, richer sensitivities, and consolidated controls."""

import altair as alt
import pandas as pd
import streamlit as st

st.set_page_config(page_title="M.R. AI Agent | V22", page_icon=":material/monitoring:", layout="wide")

try:
    import market_risk_agent_v22 as v22
...
st.session_state.setdefault("v22_active_page", "Overview")


def select_dashboard_page():
    st.session_state.v22_active_page = st.session_state.v22_navigation


def select_agent_page():
    st.session_state.v22_active_page = "Ask MR Agent"
...
stress_evolution = v22.get_stress_evolution()

portfolio_scope = v22.v14.get_portfolio_scope()
...
page = st.session_state.v22_active_page
...
stress_frame, stress_metadata = v22.build_supplied_stress_frame(selected_as_of_date)

books, _ = v22.v15.build_hierarchy()
...
st.info("V22 hierarchy filters apply transparent synthetic allocations for exploration. They are not trade-level revaluation or additive VaR.", icon=":material/info:")
...
if page == "Overview":
    ...
    st.header("VaR")
    var_change_summary = v22.get_var_change_summary(selected_as_of_date)
    ...
    pla_history = v22.build_pla_demo_history()
    pla_evaluation = v22.evaluate_pla_test()
    ...
    pnl_alert_evaluation = v22.evaluate_pnl_explain_alerts()
    ...
    st.info(
        "The 250-day desk history is deterministic synthetic V22 demo data. "
    ...
    with st.container(border=True):
        ...
        driver_values = latest_pnl[v22.DRIVER_COLUMNS].sort_values()
        explain_frame = driver_values.rename_axis("Driver").rename("value").reset_index()
        explain_frame["Driver"] = explain_frame["Driver"].replace({
            "pnl_driver_fx_delta": "FX Delta",
            "pnl_driver_ir_dv01": "IR Delta",
            "pnl_driver_vega": "Vega",
            "pnl_driver_gamma": "Gamma",
            "pnl_driver_theta": "Theta",
            "pnl_driver_cs01": "Credit spread",
            "pnl_driver_cross_gamma": "Cross Gamma",
        })
        explain_frame["Type"] = "Explained driver"
        explain_frame = pd.concat([
            explain_frame,
            pd.DataFrame([{
                "Driver": "Unexplained P&L",
                "value": float(latest_pnl["unexplained_pnl"]),
                "Type": "Unexplained",
            }]),
        ], ignore_index=True)
        explain_frame["zero"] = 0.0
        explain_order = explain_frame.reindex(
            explain_frame["value"].abs().sort_values(ascending=False).index
        )["Driver"].tolist()

        lollipop_base = alt.Chart(explain_frame).encode(
            y=alt.Y("Driver:N", title=None, sort=explain_order),
            color=alt.Color(
                "Type:N",
                title=None,
                scale=alt.Scale(
                    domain=["Explained driver", "Unexplained"],
                    range=["#2F6BFF", "#D64550"],
                ),
            ),
            tooltip=[
                alt.Tooltip("Driver:N"),
                alt.Tooltip("value:Q", title="P&L contribution", format=",.0f"),
                alt.Tooltip("Type:N"),
            ],
        )
        contribution_stems = lollipop_base.mark_rule(strokeWidth=3).encode(
            x=alt.X("zero:Q", title="P&L contribution (EUR)"),
            x2="value:Q",
        )
        contribution_points = lollipop_base.mark_circle(size=115).encode(
            x=alt.X("value:Q", title="P&L contribution (EUR)")
        )
        positive_labels = (
            lollipop_base.transform_filter(alt.datum.value >= 0)
            .mark_text(align="left", dx=9, fontWeight="bold")
            .encode(x="value:Q", text=alt.Text("value:Q", format=",.0f"))
        )
        negative_labels = (
            lollipop_base.transform_filter(alt.datum.value < 0)
            .mark_text(align="right", dx=-9, fontWeight="bold")
            .encode(x="value:Q", text=alt.Text("value:Q", format=",.0f"))
        )
        st.altair_chart(
            (contribution_stems + contribution_points + positive_labels + negative_labels)
    ...
    sensitivities = v22.get_market_sensitivities()
    ...
    def rates_sensitivity_chart(measure, unit):
        ...
        return (
            ...
            .mark_bar()
            .encode(
                x=alt.X("curve_type:N", title="Curve family", sort=["OIS", "BOR", "Inflation"]),
    ...
    with first_row[0]:
        with st.container(border=True, height="stretch"):
            ...
            st.caption("OIS, BOR and Inflation curves are grouped by family; color identifies EUR, USD, JPY and GBP.")
    with first_row[1]:
        with st.container(border=True, height="stretch"):
    ...
    material_evaluation = v22.get_material_stress_scenarios(selected_as_of_date)
    ...
    with st.expander("Scenario catalogue, including Extreme configurations", icon=":material/assignment:"):
        scenario_catalog = pd.DataFrame(v22.get_stress_scenario_catalog())
        st.dataframe(
            scenario_catalog,
            hide_index=True,
    ...
    st.header("Controls")
    limit_evaluation = v22.evaluate_all_limits()
    ...
    with st.container(border=True):
        ...
        if question:
            ...
            with st.status("Running the risk investigation...", expanded=True) as status:
                try:
                    answer = v22.ask_risk_agent(question)
                except Exception as error:
    ...
    with st.expander("Recent investigation memory", icon=":material/history:"):
        memory = v22.v9.get_recent_investigation_context()
