"""Market Risk AI V14: top navigation, business-date labels and portfolio filter."""

import pandas as pd
import streamlit as st

st.set_page_config(page_title="Market Risk AI | V14", page_icon=":material/monitoring:", layout="wide")
...
portfolio_scope = v14.get_portfolio_scope()
portfolio_ids = [row["portfolio_id"] for row in portfolio_scope["portfolios"]]

st.title("Market Risk AI")
st.caption(f"Validated demo risk run · Data as of {current_risk['date']} · V14")
...
if page == "Overview":
        ...
        if question:
            ...
            with st.chat_message("assistant", avatar=":material/analytics:"):
                with st.status("Running the risk investigation...", expanded=True) as status:
                    try:
                        answer = v14.ask_risk_agent(question)
                    except Exception as error:
    ...
    with st.expander("Recent investigation memory", icon=":material/history:"):
        memory = v14.v9.get_recent_investigation_context()
